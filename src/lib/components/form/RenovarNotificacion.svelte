<script lang="ts">
  ...
  let updateItemValues: UpdateFormItem[] = $state([
    { key: UserKeys.NOTIFICAR, value: user.notificar || "" },
  ]);
  const onUpdated = () => {
    user.notificar = updateItemValues[0].value as string;
    closeForm();
  };
...
<Card class="p-4 sm:p-6 md:p-8">
  ...
  >
    <h3 class="text-xl font-medium text-gray-900 dark:text-white">Notificar</h3>
