<script lang="ts">
    ...
    import { useFormUpdateHook, type UpdateFormItem } from "$lib/hooks/useFormUpdate.svelte";
  ...
  let updateItemValues: UpdateFormItem[] = $state([
    { key: UserKeys.DIRECCION, value: user.direccion || "" },
  ]);
  const onUpdated = () => {
    user.direccion = updateItemValues[0].value as string;
...
<Card class="p-4 sm:p-6 md:p-8">
   <form class="flex flex-col space-y-6" onsubmit={(e)=>actions.onUpdateSingleForm(updateItemValues, user.id, e)}>
    ...
    <Label class="space-y-2">
      <Input type="text" name="direccion" placeholder="" required bind:value={updateItemValues[0].value} />
    </Label>
