<script lang="ts">
  ...
  import { logout } from "$lib/services/supabase/auth";
  import {
    updateUserPassword,
    validateToken,
  } from "$lib/services/supabase/reset_password";
  ...
  async function bootstrapSession() {
    ...
    return await validateToken(code, access_token, refresh_token);
  }
  ...
  onMount(async () => {
    ...
    try {
      ...
      error_message =
        err?.message ||
        "No pudimos validar el enlace. Pide otro desde 'Olvidé mi contraseña'.";
      formVisible = false;
    } finally {
      loading = false;
    }
...
<Card class="p-4 sm:p-6 md:p-8 max-w-md mx-auto" size="md">
  ...
  {#if formVisible}
    <form class="flex flex-col gap-4" on:submit={onSubmit} novalidate>
      ...
      {#if msg}
        <div
          class="text-sm mt-1 {msgClass === 'ok'
            ? 'text-green-600'
            : msgClass === 'error'
              ? 'text-rose-600'
              : 'text-gray-500'}"
    ...
    <section class="mt-4">
      <div class="flex flex-col items-center">
        ...
        <div class="text-center text-gray-700">
          {#if verifiedEmail}
            La contraseña del usuario <strong>{verifiedEmail}</strong> ha sido restablecida
            exitosamente.
          {:else}
